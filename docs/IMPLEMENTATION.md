...
- **Catalog pagination (closes Phase 5B) — `GET /products` is now `{items, total, page, limit, pageCount}`, not a plain array.** `features/catalog/api/catalog-api.ts` has two functions now: `getCatalogProductsPage(query)` — returns the raw paginated shape, for real paginated UI (not built yet — the shop grid still fetches everything and filters client-side); `getAllCatalogProducts()` — loops every page (`limit: 100`) and flattens to an array, used by the 3 existing "I need the whole catalog" call sites (shop grid via `get-shop-data.ts`, home page sections, sitemap). **Still open:** an actual paginated `/shop` UI (page-number controls, server-side search/category/brand/sort instead of client-side filtering) — the API supports it, the UI doesn't use it yet.
...
- **Phase 5B + 6 + 2 (backend integration wave)** (2026-08-17): backend redeployed with pagination, cart-merge, admin-stats, forgot/reset-password, and image-upload all live (verified against `/api-json` directly — 47 paths, up from 42). Integrated all five in one pass — see "Backend Integration Wave" above for what changed and what's still open (a real paginated `/shop` UI is the one piece not built yet, everything else is a closed loop end-to-end).
